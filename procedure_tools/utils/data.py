...
def set_tender_period_data(
        ...
        acceleration=ACCELERATION_DEFAULT,
        period_timedelta=TENDER_PERIOD_TIMEDELTA_DEFAULT):
    period_delta = get_period_delta(acceleration, period_timedelta, seconds_buffer=TENDER_SECONDS_BUFFER * 2)
    data = data if data is not None else {'data': {'tenderPeriod': {}}}
