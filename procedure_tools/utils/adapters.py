...
class HTTPAdapter(adapters.HTTPAdapter):
    def __init__(
        self, timeout=DEFAULT_TIMEOUT, max_retries=DEFAULT_MAX_RETRIES, *args, **kwargs
    ):
...
def mount(
    session, max_reties=DEFAULT_MAX_RETRIES, status_forcelist=DEFAULT_RETRY_FORCELIST
):
